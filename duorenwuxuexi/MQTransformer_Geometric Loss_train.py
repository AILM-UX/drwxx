...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = (loss1 * loss2 * loss3).pow(1 / 3)
        loss.backward()
...
if __name__ == '__main__':
    ...
    #     device_id = torch.device("cuda:0")  # 使用名称为"GeForce_RTXTi"的GPU
    #
    #     # 获取当前 GPU 的名称
    #     device_name = torch.cuda.get_device_name(device_id)
    #
    #     # 输出当前设备的型号
    #     print(f"正在使用的 GPU: {device_name}")
    # else:
    #     print("当前没有可用的 GPU，正在使用 CPU")
    batch_size = 8


    tb_writer = SummaryWriter("MQTransformer_Geometric Loss")
    ...
    for epoch in range(epoch_num):
        ...
        if train_acc1>best_train_acc:
            torch.save(model, f'MQTransformer_Geometric Loss{epoch}.pth')
