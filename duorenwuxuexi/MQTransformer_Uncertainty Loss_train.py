...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        return total_loss
#模型训练模式将模型设置为训练模式（model.train()）。
#定义交叉熵损失函数（CrossEntropyLoss），用于分类任务。
#数据遍历与批次处理
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    sample_num = 0
    data_loader = tqdm(data_loader, file=sys.stdout)  #使用 tqdm 包装数据加载器（data_loader），显示训练进度条。
    for step, data in enumerate(data_loader): #遍历每个批次（batch），从数据加载器中提取触觉（t_1 到 t_8）和视觉（v_1 到 v_8）输入特征，以及三个任务的标签（label0, label1, label2）。
...
if __name__ == '__main__':
    ...
    tb_writer = SummaryWriter("MQTransformer_Uncertainty Loss")


    with open('E:/BaiduNetdiskDownload/duorenwuxuexi(1)/data_label/t_v_dataset_train2.pt', 'rb') as file:
        train_dataset = pickle.load(file)

    # 步骤2：从Pickle文件加载测试数据集
    with open('E:/BaiduNetdiskDownload/duorenwuxuexi(1)/data_label/t_v_dataset_test2.pt', 'rb') as file:
    ...
    for epoch in range(epoch_num):
        ...
        if train_acc1>best_train_acc:
            torch.save(model, 'MQTransformer_Uncertainty Loss_train.pth')
