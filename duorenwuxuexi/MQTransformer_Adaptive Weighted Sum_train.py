...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        def softmax_weights(raw_w1, raw_w2, raw_w3):
            weights = torch.softmax(torch.stack([raw_w1, raw_w2, raw_w3]), dim=0)
            return weights[0], weights[1], weights[2]

        w1, w2, w3 = softmax_weights(model.w1, model.w2, model.w3)
...
if __name__ == '__main__':
    ...
    tb_writer = SummaryWriter("MQTransformer_www")


    with open('../data_label/t_v_dataset_train.pt2', 'rb') as file:
        train_dataset = pickle.load(file)

    # 步骤2：从Pickle文件加载测试数据集
    with open('../data_label/t_v_dataset_test.pt2', 'rb') as file:
    ...
    for epoch in range(epoch_num):
        ...
        if train_acc1>best_train_acc:
            torch.save(model, 'MQTransformer_www.pth')
