...
from MQTransformer import swin_transformer_s_t_ada
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = model.cv1 * loss1 + model.cv2 * loss2 + model.cv3* loss3
        loss.backward()
    ...
    mean_loss1 = np.mean(batch_loss1)
    std_loss1 = np.std(batch_loss1)
    coefficient_of_variation1 = std_loss1 / mean_loss1 if mean_loss1 != 0 else 0

    mean_loss2 = np.mean(batch_loss2)
    std_loss2 = np.std(batch_loss2)
    coefficient_of_variation2 = std_loss2 / mean_loss2 if mean_loss2 != 0 else 0

    mean_loss3 = np.mean(batch_loss3)
    std_loss3 = np.std(batch_loss3)
    coefficient_of_variation3 = std_loss3 / mean_loss3 if mean_loss3 != 0 else 0

    coefficient_of_variation=coefficient_of_variation1+coefficient_of_variation2+coefficient_of_variation3

    coefficient_of_variation1=coefficient_of_variation1/coefficient_of_variation
    coefficient_of_variation2=coefficient_of_variation2/coefficient_of_variation
    coefficient_of_variation3=coefficient_of_variation3/coefficient_of_variation

        # 使用差异系数更新任务的权重系数
        # 这里你可以保存系数以便在下一轮训练时使用
        # 更新权重（这里假设是一个学习率的缩放，具体调整策略可以根据任务灵活选择）

    model.cv1 = coefficient_of_variation1
    model.cv2 = coefficient_of_variation2
    model.cv3 = coefficient_of_variation3
...
if __name__ == '__main__':
    ...
    tb_writer = SummaryWriter("MQTransformer_Coefficient of variations Weighting")
    ...
    for epoch in range(epoch_num):
        ...
        val_loss, val_acc1,val_acc2,val_acc3 = evaluate(model=model,
                                     data_loader=merged_data_test_loader,
                                     device=device,
                                     epoch=epoch)
        if train_acc1>best_train_acc:
            torch.save(model, 'MQTransformer_Coefficient of variations Weighting.pth')
        tags = ["train_loss", "train_acc1", "train_acc2", "train_acc3", "val_loss", "val_acc1","val_acc2" ,"val_acc3"  ,"learning_rate"]
